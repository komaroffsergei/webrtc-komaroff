...
6. `lib/src_langgraph_rb_node/scenarios/where_my_flight/definition.rb`
7. `lib/src_langgraph_rb_node/scenarios/where_my_flight/graph.rb`
8. `lib/src_langgraph_rb_node/scenarios/where_my_flight/nodes.rb`
...
| `runtime/graph_runner.rb` | thin adapter over `AsyncGraph::Runner`, request binding, resume loop |
...
| `scenarios/base_nodes.rb` | минимальный shared layer для node handlers |

## Authoring model

Каждый сценарий это отдельная папка с тремя файлами:
...
- topology отдельно в `graph.rb`
- node logic отдельно в `nodes.rb`
...
| Нужно изменить | Файл |
| --- | --- |
| новый сценарий | `lib/src_langgraph_rb_node/scenarios/<scenario>/` |
...
| runner loop / request binding | `lib/src_langgraph_rb_node/runtime/graph_runner.rb` |
