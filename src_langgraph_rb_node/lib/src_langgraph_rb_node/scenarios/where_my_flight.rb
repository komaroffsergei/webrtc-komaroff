      ...
      def build_graph
        AsyncGraph::Graph.new do
          node :collect_params do |state, await|
            ...
            parsed = Scenarios::WhereMyFlight.normalize_tool_params(params_resp)
            merged = Scenarios::WhereMyFlight.merge_non_empty_params(previous, parsed[:extracted])
            if Scenarios::WhereMyFlight.ready_with_any_field?(merged, %i[flight_number last_name])
              ...
              if had_pending && !FOLLOWUP_RE.match?(req[:text].to_s)
                available_scenarios = Scenarios::WhereMyFlight.filtered_router_scenarios(state, state[:current_scenario_id])
                ...
                reroute_id = Scenarios::WhereMyFlight.normalize_routing_choice(reroute_resp, available_scenarios)
          ...
          node :route_after_collect do |state|
          ...
          node :call_status_tool do |state, await|
            req = state.fetch(:req)
            ...
            message = Scenarios::WhereMyFlight.extract_llm_text(final_resp)
            message = Scenarios::WhereMyFlight.fallback_flight_status_message(tool_resp[:data]) if message.to_s.empty?
          ...
          node :route_after_tool do |state|
          ...
          node :emit_not_found_response do |state|
            { response: Runtime::Responses.done_response(state.fetch(:req), state[:response_message]) }
          end

          node :emit_failed_response do |state|
