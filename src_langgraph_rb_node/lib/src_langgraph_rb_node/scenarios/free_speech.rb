    ...
    module FreeSpeech
      ...
      def build_graph
        AsyncGraph::Graph.new do
          node :prepare_context do |state|
            entities = Scenarios::FreeSpeech.extract_entities_from_artifacts(Runtime::Util.extract_hash(state[:context_extra]))
            resolved_mode, resolved_entities, resolved_hint = Scenarios::FreeSpeech.resolve_entity_reference(
            ...
            out[:clarify_message] = Scenarios::FreeSpeech.clarify_entity_message(resolved_entities) if resolved_mode == "clarify"
            out
          end

          node :route_after_prepare do |state|
          ...
          node :clarify_reference do |state|
            { response: Runtime::Responses.done_response(state.fetch(:req), state[:clarify_message]) }
          end

          node :compose_response do |state, await|
            req = state.fetch(:req)
            primary_input = {
              task: TASK,
            ...
            primary_text = Scenarios::FreeSpeech.extract_llm_text(primary_resp)
            if primary_text.empty?
              next { response_message: FALLBACK_MESSAGE }
            end

            unless Scenarios::FreeSpeech.needs_knowledge_retry(
              ...
              next { response_message: primary_text }
            end
            ...
            secondary_resp = await.call(
              ...
              input_data: primary_input.merge(
                ...
                dialog_context: Scenarios::FreeSpeech.short_dialog_context(state[:dialog_context].to_s),
                context_artifacts: Scenarios::FreeSpeech.retry_context_artifacts(
                  context_artifacts: Runtime::Util.extract_hash(state[:context_artifacts]),
                  resolved_entities: Array(state[:resolved_entities])
                ),
                free_speech_pass: "knowledge_retry",
                retry_reason: Scenarios::FreeSpeech.retry_reason(
                  user_text: req[:text].to_s,
                  response_text: primary_text,
                  resolved_mode: state[:resolved_mode].to_s,
                  resolved_entities: Array(state[:resolved_entities])
            ...
            secondary_text = Scenarios::FreeSpeech.extract_llm_text(secondary_resp)

            primary_score = Scenarios::FreeSpeech.response_quality_score(
              user_text: req[:text].to_s,
              response_text: primary_text,
              resolved_mode: state[:resolved_mode].to_s,
              resolved_entities: Array(state[:resolved_entities])
            )
            secondary_score = Scenarios::FreeSpeech.response_quality_score(
              user_text: req[:text].to_s,
          ...
          node :emit_final_response do |state|
            { response: Runtime::Responses.done_response(state.fetch(:req), state[:response_message]) }
          end
