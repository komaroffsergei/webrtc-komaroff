  ...
  module Runtime
    module Scenarios
      module FreeSpeech
        ...
        CFG = Runtime::ConfigLoader.load_config("free_speech")
        PROMPTS = Runtime::ConfigLoader.dict_value(CFG[:prompts])
        FREE_SPEECH_TASK = Runtime::ConfigLoader.text_block(
          PROMPTS[:task],
        ...
        FREE_SPEECH_CONTEXT = Runtime::ConfigLoader.text_block(
        ...
        def primary_pass(state, io:)
          ...
          primary_resp = io.call_llm(parent: req, mode: "final_response", input_data: primary_input, constraints: { temperature: 0.4 })
          return { response: Runtime::Responses.done_response(req, "Сервис ответов временно недоступен. Попробуйте повторить запрос."), free_speech_primary_result: "terminal" } unless primary_resp[:ok]

          primary_text = extract_text(primary_resp).to_s
          needs_retry = needs_knowledge_retry(
          ...
          return { response_message: (primary_text.empty? ? "Чем могу помочь?" : primary_text), free_speech_primary_result: "done" } unless needs_retry

          {
            ...
            free_speech_primary_result: "retry"
          }
        end

        def retry_pass(state, io:)
          req = state.fetch(:req)
          secondary_input = Runtime::Util.extract_hash(state[:primary_input]).merge(
            task: "#{FREE_SPEECH_TASK}\n#{KNOWLEDGE_RETRY_TASK_SUFFIX}",
            ...
            retry_reason: state[:retry_reason].to_s
          )
          secondary_resp = io.call_llm(parent: req, mode: "final_response", input_data: secondary_input, constraints: { temperature: 0.3 })
          secondary_text = extract_text(secondary_resp).to_s

          primary_text = state[:primary_text].to_s
