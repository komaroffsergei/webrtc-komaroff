...
from llm_client import LLMClient
from mcp_client import MCPClient
from settings import AGENT_MAX_STEPS, STACK_SERVICE_NAME
from src_agent.repositories.events import log_event
...
class MCPAgent:
    def __init__(self, nc, *, llm_subject: str, events_subject: str, max_steps: int = AGENT_MAX_STEPS, db=None):
        self.current_intent_id = None
        self.nc = nc
        self.llm_client = LLMClient(nc, llm_subject=llm_subject)
        self.mcp_client = MCPClient()
    ...
    async def run(self, *, user_text: str, session_id: str) -> Dict[str, Any]:
        ...
        logger.info("MCPAgent started")
        # первый запрос (новый интент)
        # "покажи аэропорты в радиусе 100 км"
        messages: List[Dict[str, Any]] = [
            {"role": "user", "content": user_text}
        ]
        ...
        for step in range(1, self.max_steps + 1):
            ...
            #     input=None,
            #     output=llm_response,
            # )Покажи аэропорты в радиусе 100км
