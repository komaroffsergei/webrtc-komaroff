...
from src_agent.tools.tools import get_artifact_by_key, get_closed_airports, tool_context
from src_agent.utils.db import add_turn


class ListAirportsClosedScenario(Scenario):
    id = "list_airports_closed"
    title = "List closed airports"
    description = "Returns airports with status=closed from airports.json."
        ...
        *,
        ...
        with tool_context(state, self.id):
            ...
            if res.get("status") != "ok" or not isinstance(key, str):
                ...
                error_text = str(error_text) if error_text else "Failed to fetch closed airports."
        ...
        summary = f"Found {count} closed airports."
        handler = {
