...
В stack-конфиге `webrtc.drs` маршрут `/whisper` включает server-side file transcription mode для `wav/mp3/m4a/mp4`. Он загружает исходный файл в gateway, создаёт NATS file-job, worker вырезает audio-only поток, нормализует его в `mono 16k`, режет на чанки и отправляет их параллельно в file-only LinTO HTTP backend-ы, не меняя live voice pipeline `inference.whisper.*`.

Режимы ASR в этом стэке сейчас такие:
- live voice: `WebRTC -> NATS -> stt_whisper_to_nats -> LinTO websocket -> NATS`
- `/whisper` file mode: `Browser -> gateway -> NATS file job -> parallel chunked LinTO HTTP -> strict-prefix merge`
